...
export async function encryptWriteSecret(token: string, phrase: string) {
  ...
  const buf = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key.cryptoKey, te.encode(JSON.stringify({ token })))
...
export async function decryptWriteSecret(raw: string, phrase: string) {
  ...
  try {
    const buf = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: fromB64(pack.iv) }, key.cryptoKey, fromB64(pack.data))
